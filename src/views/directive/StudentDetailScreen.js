...
export default function StudentDetailScreen({ route }) {
  ...
  const [email, setEmail] = useState(
    student.email && student.email !== false ? String(student.email) : ''
  );
  ...
  const handleSave = async () => {
    ...
    if (!name.trim() || !surname.trim() || !schoolYear.trim() || !photo || !birthDate.trim() || !email.trim()) {
      mostrarAlerta(
        'Campos incompletos',
        'Nombre, Apellidos, Email, Curso, Fecha de Nacimiento y Foto son obligatorios.'
      );
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        mostrarAlerta(
          'Éxito',
          isNew ? 'Alumno creado correctamente.' : 'Datos actualizados correctamente.'
        );
        navigation.goBack();
      } else {
  ...
  const ejecutarBorrado = async () => {
    setLoading(true);
    try {
      const data = await apiClient.delete(API_ENDPOINTS.ALUMNO_BY_ID(student.id));

      if (data.success) {
        mostrarAlerta('Eliminado', 'El alumno ha sido borrado.');
        navigation.goBack();
      } else {
  ...
  const handleDelete = () => {
    ...
    if (Platform.OS === 'web') {
      ...
      Alert.alert('Eliminar Alumno', mensaje, [
    ...
    <ScrollView style={styles.container}>
      <View style={styles.formCard}>
        <Text style={styles.sectionTitle}>
          {isNew ? 'Añadir Nuevo Alumno' : 'Datos del Alumno'}
        </Text>
