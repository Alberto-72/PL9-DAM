...
export default function ScannerScreen({ route, navigation }) {
  ...
  // Lógica de escucha para navegación manual desde la lista
  useEffect(() => {
    if (route.params?.studentToValidate) {
      const datosManuales = route.params.studentToValidate;
      
      // Procesamos la validación usando el cursoCorto para la visualización de la ficha
      procesarValidacion({
        nombre: datosManuales.nombre,
        curso: datosManuales.cursoCorto || datosManuales.curso, // Aquí forzamos el código corto (1ESO, 2DAM...)
        foto: datosManuales.foto || null,
        fechaNacimiento: datosManuales.fechaNacimiento,
        tieneTransporte: datosManuales.tieneTransporte
      });

      // Limpiamos parámetros para permitir nuevas selecciones
    ...
    return (
      <View style={styles.container}>
        <View style={styles.cajaBlanca}>
          ...
          <Text style={styles.subtituloVacio}>Escanea una tarjeta o usa la búsqueda manual.</Text>
          ...
          {!escaneando ? (
            ...
            <TouchableOpacity style={[styles.botonGrande, { backgroundColor: '#EF4444' }]} onPress={() => { if (NfcManager) NfcManager.cancelTechnologyRequest(); setEscaneando(false); }}>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.tarjeta}>
        <View style={[styles.avatar, { borderColor: iconColor }]}>
          {alumno.foto ? (
            <Image 
      ...
      <TouchableOpacity style={styles.botonSiguiente} onPress={() => setAlumno(null)}>
        ...
        <Text style={styles.textoBotonSiguiente}>Listo / Siguiente</Text>
      </TouchableOpacity>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  cajaBlanca: { backgroundColor: 'white', padding: 30, borderRadius: 20, alignItems: 'center' },
  ...
  tarjeta: { backgroundColor: 'white', padding: 30, borderRadius: 20, alignItems: 'center', marginBottom: 20 },
